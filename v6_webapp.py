...
receptors = [ np.array([2, 4]),  # closest downwind
                np.array([4, 8]),  # mid-range
                np.array([6, 12]), # farther out
                np.array([3, 10]), # off-axis right
                np.array([1, 7]),  # off-axis left
             ]  # Add more receptor coordinates as needed
...
receptor_totals.to_csv("gsa_hourly/total_receptor_concentrations.csv", index=False)
...
spacing = 20                    # 20 meters spacing
wind_dir_deg = 217
downwind_deg = (wind_dir_deg + 180) % 360
downwind_rad = np.radians(downwind_deg)

# Generate grid in (i,j) then rotate
x_local = np.arange(-(n_x//2), (n_x//2)+1) * spacing
y_local = np.arange(1, n_y+1) * spacing  # Only place receptors downwind (positive y in wind direction)

xx, yy = np.meshgrid(x_local, y_local)
x_flat, y_flat = xx.ravel(), yy.ravel()

# Rotate grid to align with wind direction (downwind = 37°)
rotation_matrix = np.array([
    [np.cos(downwind_rad), -np.sin(downwind_rad)],
    [np.sin(downwind_rad),  np.cos(downwind_rad)]
])

rotated = rotation_matrix @ np.vstack([x_flat, y_flat])
receptors = [np.array([x, y]) for x, y in rotated.T]

# Visualization
plt.figure(figsize=(6, 6))
plt.scatter(*zip(*receptors), c='red', label="Receptors")
plt.quiver(0, 0, np.cos(downwind_rad), np.sin(downwind_rad), scale=5, color='blue', label=f'Wind ({downwind_deg}°)')
plt.axhline(0, color='gray', linestyle='--', linewidth=0.5)
plt.axvline(0, color='gray', linestyle='--', linewidth=0.5)
plt.gca().set_aspect('equal')
plt.grid(True)
plt.title("Receptor Grid Oriented Downwind (37°)")
plt.legend()
plt.xlabel("x (m)")
plt.ylabel("y (m)")
plt.tight_layout()
